...
class PRForm(FlaskForm):
    responsible = StringField("Ответственный: ", validators=[DataRequired("Обязательное поле"),Length(min=7, max=7, message="Длинна логина 7. начинается с v")])
    ...
    payer = SelectField("Компания плательщик: ", coerce=str, choices=['Стоков Машинное Оборудование АО',
                                                                      'Стоков Компоненты ООО', 'Стоков Конструкция ООО'])
